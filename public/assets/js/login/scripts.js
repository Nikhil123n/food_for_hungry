...
// Validate button of login form
function validate(nameInputedVal){
	...
	if(email_input=="admin@gmail.com" && password_input=="passss"){
		swal("Welcome! "+nameInputedVal, "Registeration Successful", "success");
        location.href = `index?username=${nameInputedVal}`
		return false;
	}
	else{
		swal("Error! ", "Incorret Email address or Password", "warning");
	}
}
...
function alertBox(x){
	if(x){
		swal("Welcome! "+x, "Registeration Successful", "success");
	}
	else{
		swal("Error! ", "Incorret Email address or Password", "warning");
	}
